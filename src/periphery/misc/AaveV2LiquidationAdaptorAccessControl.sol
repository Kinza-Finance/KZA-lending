...
contract AaveV2LiquidationAdaptorAccessControl is Ownable {
    ...
    function liquidateWithFlashLoan(IAaveV2Pool pool, address liquidated, address collateral, address debtToken, uint256 debtAmount, ROUTE route, bytes memory customPath) external onlyOwner {
        IPoolDataProvider dataProvider = IPoolDataProvider(provider.getPoolDataProvider());
        ...
        bytes memory params = abi.encode(address(pool), collateral, liquidated, msg.sender, route, customPath);
        pool.flashLoan(
            address(this),
            debtTokens,
            debtAmounts,
    ...
    bytes calldata params
        ...
        {
            (address AaveV2Pool, address collateralAsset, address liquidatedUser, address liquidator, ROUTE route, bytes memory customPath) = 
            abi.decode(params, (address, address, address, address, ROUTE, bytes));
        ...
        IPool pool = IPool(provider.getPool());
        IERC20(borrowedAsset).approve(address(pool), type(uint256).max);
