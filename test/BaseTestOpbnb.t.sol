...
import {ADDRESSES_PROVIDER, POOLDATA_PROVIDER, ACL_MANAGER, POOL, POOL_CONFIGURATOR, EMISSION_MANAGER, 
        ATOKENIMPL, SDTOKENIMPL, VDTOKENIMPL, TREASURY, POOL_ADMIN, ORACLE } from "test/utils/AddressesOpbnb.sol";
...
contract BaseTest is Test {
    ...
    function fork() internal {
        ...
        //uint256 mainnetFork;
        if (forkBlock == 0) {
            mainnetFork = vm.createFork(vm.envString("OPBNB_RPC_URL"));
        } else {
            mainnetFork = vm.createFork(vm.envString("OPBNB_RPC_URL"), forkBlock);
        }
